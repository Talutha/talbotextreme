...
function hourlyResets() {

  for (var channel in timers) {
    if (timers.hasOwnProperty(channel)) {
      timers[channel].chickenDinner.cooldown = false;
    }
  }
}

// resets certain things at the top of each hour. Polling every 45 seconds
// because I am paranoid and don't want to miss an hour.
function hourlyTimer() {
  let currentMinute = new Date().getMinutes();
  let reset = false;

  if (currentMinute === 0 && !reset) {
    hourlyResets();
    reset = true;

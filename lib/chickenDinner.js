...
async function serve(channel, userstate, client, message) {
  ...
  try {
    ...
    let hasEnoughPoints = await userstate.hasEnoughPoints(amount);
...
async function cancelGame(channel, client, gameState) {
  try {
    ...
    let pointsName = await CHANNELS.getPointsName(channel);
    ...
    await USERS.points.modify(channel, userID, amount);
  } catch (err) {
    console.log(err);
  }
}

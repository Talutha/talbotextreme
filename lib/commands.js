...
const DEFAULT_COMMANDS = ['dropzone', 'addcom', 'remcom', 'editcom', 'join', 'leave'];

let db, client, channel, userstate, message, elevatedUser, points;
...
async function getPoints() {
  try {
    console.log("Someone requested their points total!");
    let amount = await POINTS.getPointsAmount(db, channel, userstate);
    let pointsName = await POINTS.getPointsName(db, channel);
    client.say(channel, '@' + userstate['display-name'] + ' currently has ' + amount + ' ' + pointsName + '!');
  } catch (err) {
    console.log(err);
  }
}

...
function processCommand(db, client, channel, userstate, message) {
  ...
  if (message[0] === 'dropzone') {
    ...
    // Input: [join]
    joinChannel(db, client, channel, userstate, message);

  }
  else if (message[0] === 'leave') {

    // Input: [leave]
    leaveChannel(db, client, channel, userstate, message);
...
function joinChannel(db, client, channel, userstate, message) {
  client.join("#" + userstate['username']);
}

function leaveChannel(db, client, channel, userstate, message) {

...
const DEFAULT_COMMANDS = ['dropzone',
                          'addcom',
                          'remcom',
                          'editcom',
                          'join',
...
async function processCommand(database, cl, ch, us, msg) {
  ...
  if (message[0] === 'dropzone') {
    ...
    // THIS IS A TEST FUNCTION AND SHOULD BE REMOVED
    let timer = TIMERS.showTimers();
    timer[channel].stopTimer();
    ...
    // THIS IS A TEST FUNCTION AND SHOULD BE REMOVED
    let timer = TIMERS.showTimers();
    timer[channel].startTimer(1000, 10);

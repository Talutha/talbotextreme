...
channelInfo.streamStartDate = Date();
channelInfo.userAvatar = '';

// reference to interval id so we can stop it if needed
let refreshTimer;
...
/*
saves letious information related to a channel
...
function getInfo() {
  ...
  // set stream related information
  .then(res => {
    if (res === null) {
      channelInfo.streamIsLive = false;
    } else {

...
function getUserList(db, ch) {
  channel = ch;
  let url = 'https://tmi.twitch.tv/group/user/' + ch.slice(1) + '/chatters';
  let options = {
    uri: url,
    json: true
  };
    ...
    for (let i = 0, l = userReturn.users.length; i < l; i++) {
      db.users.find({
        ...
        db.users.update({
          ...
          // This adds 2 points every time a user is detected, DO NOT KEEP THIS IN THE PROGRAM!

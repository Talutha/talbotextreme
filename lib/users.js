...
function processForPoints(db, userList, channel, points) {
  ...
  for (let property in userList.chatters) {
    while (userList.chatters[property].length > 0) {
      let subsetList = userList.chatters[property].splice(0, 99);

      ...
      */
      ...
      if (dbReturn.length === 0) {
        ...
        if (lastUpdated > updateThreshold) {
          ...
          await points.modify(channel, dbReturn[0]['twitch_id'], points);
...
async function giveUsersPoints(db, userList, channel, points) {
  ...
  try {
    ...
    for (let user in userList) {
      ...
      points.modify(channel, userID, points);
...
async function injectPayload(userstate, channel) {
  ...
  userstate.getPointsAmount = async function() {
    let amount = await points.getAmount(channel, userID);
    return amount;
  };

  // allows modification of points directly through userstate object
  // userstate.modifyPoints(500);
  // userstate.modifyPoints(-500);
  userstate.modifyPoints = async function(amount) {
    await points.modify(channel, userID, amount);
    return;
  };

  // allows boolean check of enough points through userstate object
  userstate.hasEnoughPoints = async function(amount) {
    return points.hasEnough(channel, userID, amount);
  };
...
let points = {
  ...
  /** @description Get the amount of points a user currently has
   *  @param {channel} channel channel of user requesting points, #example
   *  @param {userID} userID userID of user, 123456
   *  @return {number}
   */
  ...
  /** @description Modify a users current amount of points
   *  @param {channel} channel channel of user to change points in, #example
   *  @param {userID} userID userID of user, 123456
   *  @param {amount} amount Amount of points to change by, -500, 500
   */
  modify: async function(channel, userID, amount) {
    try {

      let db = await DB.db;
      let currentAmount = await this.getAmount(channel, userID);
  ...
  /** @description Check if a user has enough points to spend
   *  @param {channel} channel channel of user to check points in, #example
   *  @param {userID} userID userID of user, 123456
   *  @param {amountSpending} amountSpending The amount that is being compared to the total
   *  @return {boolean}
   */
  hasEnough: async function(channel, userID, amountSpending) {
    let amountInBank = await this.getAmount(channel, userID);
